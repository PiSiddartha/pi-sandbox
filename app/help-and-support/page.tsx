import SandboxPage from "@/components/SandboxPage";

export default function HelpAndSupportPage() {
  return (
    <SandboxPage
      title="Help & Support"
      description={[
        "Help & Support is your central place for documentation, FAQs, and support tickets. You can open new tickets, track P1 incidents, and get help from the Pay-in-de-li team.",
        "In the full Pi Repository this section is connected to the ticketing system and live help resources. In the Sandbox you can learn the layout and flows from the tutorial video.",
      ]}
    />
  );
}
